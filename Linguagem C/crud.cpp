...
void alterar(){
	...
	while (fread (&produtos, sizeof(produtos), 1, arq)){
		if (cod == produtos.codigo){
			...
			//MOVE O PONTEIRO ATE A POSICAO QUE DESEJAMOS ALTERAR
...
void organizar(){
	...
	int teste = 0;
	teste = system( "ren BackupProdutos.pro produtos.pro" );
...
void copiar(){
	...
	char caminho[250];
	printf("\nInforme o diretorio e nome do arquivo onde deseja armazenar o backup ( ex.: C\\\\:produtos.bkp ) \n");
	fflush(stdin);
	gets(caminho);

	FILE *arqB = fopen(caminho, "wb");
	if (arqB == NULL){
		printf("Erro na abertura!");
		return;
	}
...
void restaurar(){
	...
	printf("\nInforme o caminho completo até o arquivo que deseja restaurar ( ex.: C\\\\:produtos.bkp ) \n");
	fflush(stdin);
	gets(caminho);

	strcpy( comando, "ren " );
	strcpy( comando, caminho );
	strcpy( comando, " produtos.pro" );

	int teste = 0;
	teste = system(comando);
